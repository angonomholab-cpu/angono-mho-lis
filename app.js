...
// ==========================================
// WAG KALIMUTAN: ILAGAY ANG IYONG GOOGLE SCRIPT URL DITO
...
let selectedIds = new Set(); // For Registry Batch Print
...
function applyPermissions() {
    ...
    const floatBtn = document.getElementById('menu-toggle-btn');
    ...
    if (role === 'ADMIN' || role === 'STAFF') {
        ...
        if(navReg) navReg.style.display = 'flex';
        if(floatBtn) floatBtn.style.display = 'none'; // Viewers don't need the float actions
    }
}
...
function updateSummary() { 
    const container = document.getElementById('order-summary'); container.innerHTML = ''; 
...
function setSelectValue(id, val) {
    ...
    const searchVal = String(val).toUpperCase().trim();
    for (let i = 0; i < el.options.length; i++) { 
        if (el.options[i].value.toUpperCase().trim() === searchVal || el.options[i].text.toUpperCase().trim() === searchVal) { 
...
function generateSmartID() {
    ...
    let initials = "XX"; if(name) { const p = name.split(" "); initials = p.length > 1 ? p[0][0] + p[p.length-1][0] : name.substring(0,2); }
    document.getElementById('finalPatientId').value = `MHOA-${bday}-${initials}${Math.floor(Math.random()*90+10)}`;
}

// DIRECT SEARCH & PATIENT HISTORY INJECTION
let searchTimeout; 
...
async function fetchHistory(id) {
    const section = document.getElementById('history-section'); const list = document.getElementById('history-list');
    section.style.display = 'block'; list.innerHTML = '<div style="text-align:center; color:var(--pri);"><i class="ph ph-spinner ph-spin"></i> Retrieving records...</div>';
    try {
        const res = await apiGet("getPatientHistory", { patientId: id });
        if (res.status === 'success' && res.data.length > 0) {
            list.innerHTML = res.data.map(h => `<div class="history-card"><div><div class="h-test">${h.test}</div><div class="h-date">${new Date(h.date).toLocaleDateString()}</div></div><span class="badge badge-neutral" style="font-size:0.6rem;">${h.result}</span></div>`).join('');
        } else { list.innerHTML = '<div class="text-muted text-xs text-center">No previous records found.</div>'; }
...
function clearForm() {
    ...
    // Reset from Pending Edit Mode if active
    ...
    const editBox = document.getElementById('edit-pending-details-box');
...
async function finalSubmit() {
  ...
  try {
      ...
      if (res.status === "success") {
          ...
          clearForm(); 
          if (currentUser.role !== 'ENCODER') loadPendingData(); // Auto refresh pending
...
// ==========================================
// 5. FULL SCREEN PENDING EDIT LOGIC
// ==========================================
function editPendingFull(id) {
    ...
    // Transform Right Pane
    const rightPane = document.getElementById('entry-right-pane');
    rightPane.innerHTML = `<label class="field-label" style="color:var(--warning); margin-bottom:10px; font-size:0.8rem;">Editing Specific Test: ${item.test}</label><textarea id="edit-pending-details-box" class="form-input" style="min-height: 250px; font-family: monospace; font-size: 0.8rem;">${JSON.stringify(d, null, 2)}</textarea>`;
    
    const saveBtn = document.getElementById('save-btn-action');
    saveBtn.innerHTML = '<i class="ph ph-check-circle"></i> Update Pending Record';
    saveBtn.onclick = submitPendingUpdate; saveBtn.style.background = 'var(--warning)'; saveBtn.style.color = 'white';
}
...
async function submitPendingUpdate() {
    ...
    try {
        await apiPost("updatePatientAndTestDetails", { testId: editingPendingId, patientId: item.patientId, newName: document.getElementById('p_name').value, newTestType: item.test, newJsonDetails: document.getElementById('edit-pending-details-box').value });
        cancelEditPending(); loadPendingData();
    } catch(e) { alert("Error: " + e); } finally { btn.innerHTML = oldTxt; btn.disabled = false; }
}

// ==========================================
// 6. QUICK SEARCH MODAL LOGIC
// ==========================================
...
async function viewQuickProfile(p) {
    document.getElementById('quick-profile-view').style.display = 'block';
    document.getElementById('qs-name').innerText = p.name;
    document.getElementById('qs-meta').innerText = `${p.id} | ${p.age} yrs | ${p.sex} | ${p.facility}`;
    
    const list = document.getElementById('qs-history-list');
    list.innerHTML = '<div style="text-align:center;"><i class="ph ph-spinner ph-spin"></i> Retrieving history...</div>';
    try {
        const res = await apiGet("getPatientHistory", { patientId: p.id });
        if (res.status === 'success' && res.data.length > 0) {
            list.innerHTML = res.data.map(h => `
                <div class="history-card" onclick="printDirect(event, '${p.id}', '${h.test}')" style="cursor:pointer; border-left: 3px solid var(--pri);" title="Click to Print this Result">
                    <div><div class="h-test">${h.test}</div><div class="h-date">${h.date}</div></div>
                    <div style="display:flex; align-items:center; gap:10px;">
                        <span class="badge badge-neutral" style="font-size:0.6rem;">${h.result}</span>
                        <i class="ph ph-printer" style="color:var(--pri);"></i>
                    </div>
                </div>
            `).join('');
        } else { list.innerHTML = '<div class="text-muted text-xs">No lab records found.</div>'; }
    } catch(e) {}
}
...
function renderLists() {
    ...
    cList.innerHTML = fComp.map(item => {
        return `<div class="completed-card" onclick="printDirect(event, '${item.id}', '${item.test}')" title="Click to print">
            <div style="overflow:hidden;"><div class="pc-name">${item.name}</div><div class="pc-meta">${item.test}</div></div>
            <i class="ph ph-printer" style="color: var(--success); font-size: 1.2rem;"></i>
        </div>`;
    }).join('');
            ...
            sorted.forEach(row => {
                ...
                hMap.forEach(c => {
                    ...
                    if (c.original.toLowerCase().includes('result') || c.original.toLowerCase().includes('code') || c.original.toLowerCase().includes('diagnosis')) {
                        ...
                        else if (vU==="RR") style = "res-dark-red"; else if (vU==="TT"||vU==="TI") style = "res-warning";
...
function filterRegistryTable() {
    const s = document.getElementById('regSearch').value.toLowerCase(); const m = document.getElementById('monthFilter').value.toLowerCase();
    document.querySelectorAll('#regTableBody tr').forEach(tr => { const text = tr.textContent.toLowerCase(); tr.style.display = (text.includes(s) && text.includes(m)) ? "" : "none"; });
}
...
function renderSettings(data) { const uList = document.getElementById('list-users'); if (!data.users || data.users.length === 0) { uList.innerHTML = '<div style="text-align:center; color:var(--text-muted);">No users found.</div>'; return; } const myRole = (typeof currentUser !== 'undefined' && currentUser.role) ? String(currentUser.role).toUpperCase() : ""; const isAdmin = (myRole === 'ADMIN'); uList.innerHTML = data.users.map(u => { const status = String(u.status || "").toUpperCase(); const isPending = (status === 'PENDING'); let statusDisplay = ''; let cardBorder = 'border-color: var(--border-color);'; if (isPending && isAdmin) { cardBorder = 'border-color: var(--warning); background: var(--warning-bg);'; statusDisplay = `<div style="display:flex; gap:8px; margin-top:8px;"><button onclick="decideUser('${u.username}', 'APPROVE')" class="btn btn-primary" style="padding: 4px 8px; font-size: 0.7rem; background: var(--success);"><i class="ph ph-check"></i></button><button onclick="decideUser('${u.username}', 'REJECT')" class="btn btn-danger" style="padding: 4px 8px; font-size: 0.7rem;"><i class="ph ph-x"></i></button></div>`; } else { let badgeClass = status === 'ACTIVE' ? 'badge-negative' : (status === 'REJECTED' ? 'badge-positive' : 'badge-warning'); statusDisplay = `<div style="margin-top:8px;"><span class="badge ${badgeClass}">${u.status || 'ACTIVE'}</span></div>`; } let editBtn = isAdmin ? `<button onclick="openEditUser('${u.username}', '${u.role}', '${u.status}')" class="btn-icon"><i class="ph ph-pencil-simple"></i></button>` : ''; return `<div class="patient-card" style="margin-bottom: 8px; ${cardBorder} flex-direction: row; justify-content: space-between; align-items: flex-start;"><div><div class="pc-name">${u.fullname || u.username}</div><div class="pc-meta" style="margin-top:2px;">@${u.username} • ${u.role} • ${u.facility}</div>${statusDisplay}</div>${editBtn}</div>`; }).join(''); }
let currentEditTarget = "";
function openEditUser(username, role, status) { currentEditTarget = username; document.getElementById('edit-username-display').innerText = "@" + username; document.getElementById('edit-role-select').value = role; document.getElementById('edit-status-select').value = status; document.getElementById('edit-user-modal').style.display = 'flex'; }
function closeEditModal() { document.getElementById('edit-user-modal').style.display = 'none'; }
async function saveUserChanges() { const newRole = document.getElementById('edit-role-select').value; const newStatus = document.getElementById('edit-status-select').value; const btn = document.getElementById('btn-save-user'); const oldText = btn.innerText; btn.innerHTML = '<i class="ph ph-spinner ph-spin"></i> Saving...'; btn.disabled = true; try { await apiPost("updateUser", { targetUsername: currentEditTarget, newRole: newRole, newStatus: newStatus, adminRole: currentUser.role }); closeEditModal(); loadSettingsData(); } catch(e) {} finally { btn.innerText = oldText; btn.disabled = false; } }
async function decideUser(username, action) { if(!confirm(action + " access for " + username + "?")) return; try { await apiPost("approveUser", { targetUsername: username, userAction: action, adminRole: currentUser.role }); loadSettingsData(); } catch(e) {} }
async function saveUser() { const user = { u: document.getElementById('u_user').value, p: document.getElementById('u_pass').value, role: document.getElementById('u_role').value, fac: document.getElementById('u_facility').value, name: document.getElementById('u_user').value }; if(!user.u || !user.p || !user.role) { alert("Please fill all fields."); return; } const btn = document.querySelector('#user-form button'); const oldText = btn.innerText; btn.innerHTML = "SAVING..."; btn.disabled = true; try { await apiPost("saveNewUser", { data: { username: user.u, password: user.p, facility: user.fac, role: user.role, fullName: user.name, roleCheck: currentUser.role }}); toggleForm('user-form'); document.getElementById('u_user').value = ""; document.getElementById('u_pass').value = ""; loadSettingsData(); } catch(e) {} finally { btn.innerText = oldText; btn.disabled = false; } }
let globalFacilityList = []; async function loadFacilities() { try { const res = await apiGet("getFacilityList"); globalFacilityList = res.data || []; renderFacilityList(); } catch(e) {} }
function renderFacilityList() { const container = document.getElementById('list-facilities'); const dropdown = document.getElementById('u_facility'); if(dropdown) { while (dropdown.options.length > 1) { dropdown.remove(1); } } if(container) { container.innerHTML = globalFacilityList.map((f, index) => `<div class="patient-card" style="margin-bottom: 8px; border-left: 3px solid var(--warning); flex-direction: row; justify-content: space-between; align-items: flex-start;"><div><div class="pc-name">${f.name}</div><div class="pc-meta" style="margin-top:2px;">${f.address || ""}</div>${ f.person ? `<div class="pc-meta" style="margin-top:2px; color:var(--pri);">${f.person} (${f.number})</div>` : '' }</div><div style="display:flex; gap:4px;"><button onclick="editFacility(${index})" class="btn-icon"><i class="ph ph-pencil-simple"></i></button><button onclick="deleteFacility(${index})" class="btn-icon" style="color:var(--danger);"><i class="ph ph-trash"></i></button></div></div>`).join(''); } globalFacilityList.forEach(f => { if(dropdown) { let o = document.createElement('option'); o.value = f.name; o.innerText = f.name; dropdown.appendChild(o); } }); }
let editingFacilityIndex = -1; async function handleSaveFacility() { const name = document.getElementById('f_name').value; if (!name) return; const newItem = { name: name, address: document.getElementById('f_address').value, person: document.getElementById('f_person').value, number: document.getElementById('f_number').value }; if (editingFacilityIndex >= 0) { globalFacilityList[editingFacilityIndex] = newItem; editingFacilityIndex = -1; } else { globalFacilityList.push(newItem); } renderFacilityList(); clearFacilityForm(); toggleForm('fac-form'); }
...
function deleteFacility(index) { if(!confirm("Remove facility?")) return; globalFacilityList.splice(index, 1); renderFacilityList(); }
...
let globalStaffList = []; let editingStaffIndex = -1; async function loadStaff() { try { const res = await apiGet("getStaffList"); globalStaffList = res.data || []; renderStaffList(); } catch(e) {} }
function renderStaffList() { const container = document.getElementById('staffListContainer'); if (!container) return; if (globalStaffList.length === 0) { container.innerHTML = '<div style="text-align:center; color:var(--text-muted);">No staff found.</div>'; return; } container.innerHTML = globalStaffList.map((s, index) => { let previewUrl = cleanDriveLink(s.sigUrl); const sigBadge = previewUrl ? `<img src="${previewUrl}" style="height:30px; border:1px solid var(--border-color); border-radius:4px; padding:2px; object-fit:contain;" onerror="this.style.display='none'">` : `<span class="badge badge-neutral">No Sig</span>`; return `<div class="patient-card" style="margin-bottom: 8px; border-left: 3px solid var(--danger); flex-direction: row; justify-content: space-between; align-items: center;"><div style="flex:1;"><div class="pc-name">${s.name}</div><div class="pc-meta" style="margin-top:2px;">${s.role} • Lic: ${s.license || "N/A"}</div></div><div style="margin-right: 12px;">${sigBadge}</div><div style="display:flex; gap:4px;"><button onclick="editStaff(${index})" class="btn-icon"><i class="ph ph-pencil-simple"></i></button><button onclick="deleteStaff(${index})" class="btn-icon" style="color:var(--danger);"><i class="ph ph-trash"></i></button></div></div>`; }).join(''); }
...
async function deleteStaff(index) { if(!confirm("Remove staff?")) return; globalStaffList.splice(index, 1); renderStaffList(); try { await apiPost("saveStaffData", { staffArray: globalStaffList }); } catch(e) {} }
