...
function processNtpResultsClient(p) {
    ...
    // 🟢 3. Sa DSSM, automatic na Diagnosis at New kapag may result ang Smear 1 o Smear 2
    const hasSmear = Boolean((p.smear1 && String(p.smear1).trim() !== "") || (p.smear2 && String(p.smear2).trim() !== ""));
    if (p.isDSSM && hasSmear) {
        if (!p.reason || p.reason === 'Follow-up') p.reason = 'Diagnosis';
        if (!p.history || String(p.history).toUpperCase() !== 'RETREATMENT') p.history = 'New';
    }
    if (p.results) {
        p.results.forEach(r => {
...
function localGenerateNTPHtml(patientsArray) {
    ...
    return `<!DOCTYPE html><html><head><title>NTP Form 2A Batch</title><style>
        ...
        @media print { 
            ...
            .page-container { width: 200mm !important; min-height: 275mm !important; margin: 0 auto !important; padding: 10mm !important; border: none !important; box-shadow: none !important; overflow: visible !important; page-break-after: always; page-break-inside: avoid; box-sizing: border-box !important; } 
