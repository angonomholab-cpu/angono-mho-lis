...
let isExistingPatient = false; // SMART LOCK FLAG
...
function openTestDetails(id) {
    ...
    area.innerHTML = `
        <div style="font-weight: 700; color: var(--pri); margin-bottom: 8px; display: flex; align-items: center; gap: 6px; font-size: 0.8rem;">
...
function cancelDetail() { 
    ...
    document.getElementById('test-buttons-container').style.display = 'flex'; 
}
...
function confirmDetail(id) {
   ...
   if(id === 'mtb') details = { "History of Treatment": document.getElementById('gx_hist').value, "Source of Request": document.getElementById('gx_src').value, "X-Ray Result": document.getElementById('gx_xray').value };
       ...
       if(document.getElementById('dn_duo_check') && document.getElementById('dn_duo_check').checked) subSelected.push('Dengue Duo');
   }
...
function generateSmartID() {
    if(isExistingPatient) return; // Smart Lock
...
async function runDirectSearch(q) {
  ...
  stat.style.display='block'; // Show searching indicator
      ...
      try {
          ...
          if (res.status === "success" && res.data.length > 0) {
              ...
              res.data.forEach(p => {
                  ...
                  div.innerHTML = `<div style="font-weight:600;">${p.name} <span style="font-size:0.6rem; background:var(--pri-light); color:var(--pri); padding:2px 4px; border-radius:4px; margin-left:4px;">Returning Patient</span></div><div style="font-size:0.75rem; color:var(--text-muted);">${p.age}y | ${p.sex} | ${p.facility}</div>`;
...
function renderLists() {
    ...
    pList.innerHTML = fPending.map(item => {
        ...
        if(!isEditing) {
            ...
            return `<div class="pending-card" style="border-color:var(--warning);">
...
function getResultTemplate(code, safeId, item) {
 ...
 switch (code) {
     ...
     case 'DENGUE': 
        let duoHtml = '';
        if (req.includes('DUO')) { duoHtml = select('IgG', 'IgG', ['Negative','Positive']) + select('IgM', 'IgM', ['Negative','Positive']); }
